...
namespace Dreamteck.Splines
{
    ...
    public partial class SplineEditor : Editor 
    {
        ...
        void OnEnable()
        {
            ...
            if (EditorPrefs.HasKey("SplineEditor_surfaceLayerMask")) surfaceLayerMask = EditorPrefs.GetInt("SplineEditor_surfaceLayerMask");
            if (EditorPrefs.HasKey("SplineEditor_editSpace")) editSpace = EditorPrefs.GetInt("SplineEditor_editSpace");
        ...
        void OnSceneGUI()
        {
            ...
            else if (Tools.current != Tool.None)
            {
                ...
                SplineEditorGUI.Label(new Rect(EditorGUIUtility.currentViewWidth / 2f - 80 * SplineEditorGUI.scale, 5 * SplineEditorGUI.scale, 160 * SplineEditorGUI.scale, 40 * SplineEditorGUI.scale), "Editing the Transform");
            }
            Handles.EndGUI();
        ...
        void PointMenu()
        {
            ...
            {
                avgPoint.SetTangentPosition(EditorGUILayout.Vector3Field("Tangent 1", avgPoint.tangent));
                yPosition += 22;
                avgPoint.SetTangent2Position(EditorGUILayout.Vector3Field("Tangent 2", avgPoint.tangent2));
                yPosition += 22;
            }
            if (computer.space == SplineComputer.Space.Local) avgPoint.SetPosition(computer.transform.TransformPoint(avgPoint.position));
            if (!computer.is2D)
            {
