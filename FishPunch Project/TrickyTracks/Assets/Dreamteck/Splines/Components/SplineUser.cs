    ...
    public class SplineUser : MonoBehaviour {
        ...
        public bool samplesAreLooped
        {
            get
            {
                ...
                return rootUser.computer.isClosed && clipFrom >= clipTo;
            }
        }
        ...
        private void SampleComputer()
        {
           ...
           // if (_span != span) fullIterations = DMath.CeilInt(_span / moveStep) + 1;
            ...
            if (uniformSample)
            {
                ...
                for (int i = 1; i < fullIterations-1; i++) _samples[i] = _address.Evaluate(_address.Travel(_samples[i-1].percent, lengthStep, Spline.Direction.Forward, fullIterations));
                if (computer.isClosed) _samples[samples.Length - 1] = new SplineResult(_samples[0]);
                else _samples[_samples.Length - 1] = _address.Evaluate(1.0);
        ...
        private void GetClippedSamples()
        {
            ...
            if (_clippedSamples.Length != clippedIterations) _clippedSamples = new SplineResult[clippedIterations];
